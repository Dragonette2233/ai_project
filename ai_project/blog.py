...
def create_update(id=None):
    ...
    if request.method == 'POST':
        ...
        if id:
            ...
            existence_post = Blog.query.get(post.id)
            existence_post.title = title
            existence_post.body = body

            if file.filename != '' and existence_post.photo != 'default.jpg':
                post_filename = os.path.join(current_app.instance_path, 'post_photos', existence_post.photo)
                os.system(f'rm {post_filename}')
            
            if file.filename != '':

                existence_post.photo = filename
